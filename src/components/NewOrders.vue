  ...
  <div>
    <template>
      <b-container fluid>
        ...
        <b-modal id="itemsModal" size="lg" ok-only :title="moment(itemsModal.order.dateCreated).format('Do MMMM YYYY, h:mm:ss a')">
          <template>
...
export default {
  ...
  computed: {
    ...mapGetters(['getNewOrders', 'getShippedOrders']),
    filteredDate: {
      get() {
        if (
  ...
  methods: {
    openitemsModal(item, button) {
      ...
      this.$root.$emit('bv::show::modal', 'itemsModal', button)
    },
